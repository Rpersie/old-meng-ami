...
export ENC_KERNELS=( 9 )        # Assume square kernels (AxA)
# export ENC_POOLS=( 3 )          # Pool only in frequency; no overlap. Use 0 to indicate no pooling
...
export DEC_KERNELS=( 9 )        # Assume square kernels (AxA)
# export DEC_POOLS=( 3 )          # Pool only in frequency; no overlap. Use 0 to indicate no pooling
...
export EXPT_NAME="ENC_C${ENC_CHANNELS_DELIM}_K${ENC_KERNELS_DELIM}_F${ENC_FC_DELIM}/LATENT_${LATENT_DIM}/DEC_F${DEC_FC_DELIM}_C${DEC_CHANNELS_DELIM}_K${DEC_KERNELS_DELIM}/ACT_${ACTIVATION_FUNC}/OPT_${OPTIMIZER}_LR_${LEARNING_RATE}_EPOCHS_${EPOCHS}_BATCH_${BATCH_SIZE}_DEBUG_${DEBUG_MODEL}"
