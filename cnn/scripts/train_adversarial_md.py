...
def run_training(run_mode):
    ...
    best_ckpt_path = os.path.join(model_dir, "best_cnn_fc_%s_act_%s_%s_ratio%s_md.pth.tar" % (os.environ["ADV_FC_DELIM"],
                                                                                              adv_activation,
                                                                                              run_mode,
                                                                                              str(noise_ratio)))
    ...
    # Set up optimizers for each decoder, as well as a shared encoder optimizer and adversary optimizer
    ...
    def save_checkpoint(state_obj, is_best, model_dir):
        if not save_best_only:
            ckpt_path = os.path.join(model_dir, "ckpt_cnn_fc_%s_act_%s_%s_ratio%s_md_%d.pth.tar" % (os.environ["ADV_FC_DELIM"],
                                                                                                    adv_activation,
                                                                                                    run_mode,
                                                                                                    str(noise_ratio),
                                                                                                    state_obj["epoch"]))
...
if len(sys.argv) >= 2:
    run_mode = sys.argv[1]
    if len(sys.argv) >= 3:
        profile = True if sys.argv[2] == "profile" else False
...
if profile:
    ...
    profile_output_path = os.path.join(profile_output_dir, "train_adversarial_fc_%s_act_%s_%s.prof" % (os.environ["ADV_FC_DELIM"],
                                                                                                       os.environ["ADV_ACTIVATION"],
                                                                                                       run_mode))
    cProfile.run('run_training(run_mode)', profile_output_path) 
else:
