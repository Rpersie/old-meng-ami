...
def run_training(run_mode):
    ...
    def train(epoch):
        ...
        while batches_processed < total_batches:
            ...
            class_truth = Variable(torch.FloatTensor(np.zeros((batch_size, 1))) if decoder_class == "ihm" else torch.FloatTensor(np.ones((batch_size, 1))))
            if on_gpu:
    ...
    def test(epoch, loaders):
        ...
        for decoder_class in decoder_classes:
            for feats, targets in loaders[decoder_class]:
                ...
                class_truth = Variable(torch.FloatTensor(np.zeros((batch_size, 1))) if decoder_class == "ihm" else torch.FloatTensor(np.ones((batch_size, 1))), volatile=True)
                if on_gpu:
                    class_truth = class_truth.cuda()
                loss = adversarial_loss(class_prediction, class_truth)
...
if run_mode != "ae":
    print("Currently only supporting run mode 'ae'; stopping", flush=True)
    sys.exit(1)
