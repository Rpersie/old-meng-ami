...
sys.path.append("./")
sys.path.append("./dnn")
from dnn_md import DNNVariationalMultidecoder
...
model = DNNVariationalMultidecoder(feat_dim=feat_dim,
...
def vae_loss(recon_x, x, mu, logvar, recon_only=False):
    MSE = nn.MSELoss()(recon_x.view(-1, input_dim), x.view(-1, input_dim))
